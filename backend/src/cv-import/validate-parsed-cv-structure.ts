...
export function validateParsedCVStructure(result: CVParseResult): void {
  for (const workExperience of result.workExperiences) {
    if (isBlank(workExperience.company)) {
      throw new Error("El CV parseado tiene una work experience con company vacío");
    }
    if (isBlank(workExperience.role)) {
      throw new Error("El CV parseado tiene una work experience con role vacío");
    }
    if (isInvalidDate(workExperience.startDate)) {
      throw new Error(`El CV parseado tiene un startDate inválido: "${workExperience.startDate}"`);
    }
    if (workExperience.endDate !== null && isInvalidDate(workExperience.endDate)) {
      throw new Error(`El CV parseado tiene un endDate inválido: "${workExperience.endDate}"`);
    }
    for (const bullet of workExperience.bullets) {
      if (isBlank(bullet.text)) {
        throw new Error(`El CV parseado tiene un bullet vacío en la work experience "${workExperience.company}"`);
      }
    }
  }
}
