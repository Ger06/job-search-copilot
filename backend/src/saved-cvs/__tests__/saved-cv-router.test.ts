...
describe("GET /saved-cvs/:id", () => {
  it("devuelve 200 con el SavedCV cuando existe", async () => {
    ...
    const response = await sessionRequest(app).get(`/saved-cvs/${savedCV.id}`);
  ...
  it("devuelve 404 si el id no existe", async () => {
    ...
    const response = await sessionRequest(app).get("/saved-cvs/no-existe");

    expect(response.status).toBe(404);
    expect(response.body.error).toBeDefined();
  });

  it("devuelve 404 si el SavedCV existe pero es de otra sesión", async () => {
    const { app, deps } = setUpApp();
    const jobDescription = await createJobDescription(
      { company: "Acme Corp", role: "Backend Engineer", rawText: "Buscamos un Backend Engineer" },
      TEST_SESSION_ID,
    ...
    const response = await request(app).get(`/saved-cvs/${savedCV.id}`).set("X-Session-Id", "otra-sesion");
