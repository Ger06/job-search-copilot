...
export async function createApplication(
  ...
  if (input.savedCvId != null && (await savedCVRepository.findById(input.savedCvId)) === undefined) {
...
async function findApplicationOrThrow(id: string, repository: ApplicationRepository): Promise<Application> {
...
export async function updateApplicationStatus(
  ...
  const application = await findApplicationOrThrow(id, repository);
  return repository.update({ ...application, status, updatedAt: new Date() });
}
...
export async function linkSavedCVToApplication(
  ...
  const application = await findApplicationOrThrow(id, repository);
  return repository.update({ ...application, savedCvId, fitScore, updatedAt: new Date() });
}

export async function listApplications(repository: ApplicationRepository): Promise<Application[]> {
...
export async function listApplicationsAsCsv(
  ...
  const rows = await Promise.all(
    applications.map(async (application) => {
      // No hay delete de JobDescription en el proyecto todavía, así que la
      // referenciada por jobDescriptionId siempre existe.
      const jobDescription = (await jobDescriptionRepository.findById(application.jobDescriptionId)) as JobDescription;
      return [
