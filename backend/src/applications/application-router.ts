...
export function createApplicationRouter(deps: {
  ...
  router.get("/applications", async (_req, res, next) => {
    try {
      const applications = await listApplications(deps.applicationRepository);
  ...
  router.get("/applications/export.csv", async (_req, res, next) => {
    try {
      const csv = await listApplicationsAsCsv(deps.applicationRepository, deps.jobDescriptionRepository);
      res.status(200);
  ...
  router.patch("/applications/:id/details", async (req, res, next) => {
    try {
      ...
      const application = await updateApplicationDetails(req.params["id"] as string, changes, deps.applicationRepository);
